(defvar *template-renderer*
  (make-instance 'frelia-template:template-renderer))

(defmacro deftemplate (name lambda-list &body body)
  `(frelia-macro:register-template *macro-renderer*
                        ,(alexandria:make-keyword (symbol-name name))
                        (lambda ,lambda-list ,@body)))

(deftemplate html-base-template (&key title head-block body-block)
  "HTML base template."
  `(:html :lang "en"
          (:head
           (:meta :charset "UTF-8")
...
(deftemplate base-template (&key title head-block body-block)
  "Site base template."
  (html-base-template
   :title title
...
(deftemplate content-page-template (&key title head-block content-block)
